...
@onready var animation_tree= get_node("AnimationTree2")
@onready var playback=animation_tree.get("parameters/playback")# playback variable
@onready var player_mesh = get_node("Knight")

@export var gravity: float = 9.8
@export var jump_force: int = 9
@export var walk_speed: int = 3 # 3
@export var run_speed: int = 10 # 10
...
func attack1():
	if(idle_node_name in playback.get_current_node()) or (walk_node_name in playback.get_current_node()) or (run_node_name in playback.get_current_node()) or (jump_node_name in playback.get_current_node()):
...
func damage_toplayer(damage: int):
	if !just_hit:
		...
		tween.tween_property(self,"global_position",global_position-(direction/1.5), .2 )
